...
def apply_screen(in_file, out_file, screen):
    with fits.open(in_file) as hdu_in:
        hdu_in[0].data = np.array(screen * hdu_in[0].data, dtype=np.float32)

        hdu_in.writeto(out_file, overwrite=True)


def derive_correction_screen(hdu_in, w, poly_order, correct_ra):
    print(f"Calculation correction screen for shape {img_shape}")

    # create an array but don't set the values (they are random)
    indexes = np.empty(
        (hdu_in[0].data.shape[m] * hdu_in[0].data.shape[n], 2), dtype=int,
    )
    ...
    for i in range(0, poly_order + 1):
        dec_corr += P_dec[i] * pow(dec, poly_order - i)
    dec_corr = 10 ** dec_corr

    if correct_ra is True:
        for i in range(0, poly_order + 1):
            ra_corr += P_ra[i] * pow(ra - ra_cent, poly_order - i)
        ra_corr = 10 ** ra_corr
    ...
    corr_apply = corr_apply.reshape(hdu_in[0].data.shape[m], hdu_in[0].data.shape[n])
...
if __name__ == "__main__":
    ...
    group3.add_argument(
        "--cores",
        default=1,
        help="Number of sub-process to be used when apply correction screens to images",
    )
    ...
    if results.do_rescale is True:
        ...
        # List of args to throw into multi-processing
        args_list = []

        # First pass is to create the screens
        for item, fitsimage in enumerate(infiles):
            ...
            for ext in extlist:
                ...
                if (not os.path.exists(outfits)) or results.overwrite is True:
                    ...
                    with fits.open(infits) as hdu_in:
                        if hdu_in[0].data is None:
                            ...
                            metafits = glob.glob(
                                "{0}/{1}*metafits*".format(path, fl[0:10])
                            )
                            metafits = metafits[0]
                            meta = fits.getheader(metafits)
                            ra_cent = meta["RA"]
                            ...
                            if img_shape not in corr.keys():
                                corr_apply = derive_correction_screen(
                                    hdu_in, w, results.poly_order, results.correct_ra
                                )
                                print(f"Caching correction screen for {img_shape}")
                                corr[img_shape] = corr_apply

                            args_list.append((infits, outfits, corr[img_shape]))

        # and now apply the screens in parallel fashion
        with Pool(processes=results.cores) as pool:
            pool.starmap(apply_screen, args_list)
